...
As a Teaching Assistant (TA), you need to query the name and id of all the students whose best grade comes from Option 3, sorted based on
the first 3 characters of their name. If the first 3 characters of two names are the same, then the student with the lower ID 
value comes first.*/

CREATE PROCEDURE gradeDistribution()
BEGIN
	/* Write your SQL here. Terminate each statement with a semicolon. */
    select Name, ID from (select Name, ID, (0.25*Midterm1+0.25*Midterm2+0.5*Final) as option1,
            (0.5*Midterm1+0.5*Midterm2) as option2, Final as option3
            from Grades) as result
    where Option3 > option1 or option3> option2
